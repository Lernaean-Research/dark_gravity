...
# Library and details for cluster: Abell 2744
# Modeled June 30 2013; last update August 30 2013
...
# Library includes 2 model and error subdirectories, each corresponding to a different mass modeling method (or parametrization; see below for more details).
  Each of the 2 subdirectories/models contains 14 files. For each of the two methods we use to model the clusters, we supply here:
 - best-fit deflection angle maps (x and y components), scaled to a source redshift specified therein, in units of pixels (with 0.06"/pix)
...
#Specific details for the models for Abell 2744

LTM: here we use a 2D polynomial Spline interpolation smoothing. We leave eight galaxies to be freely weighted - the two main BCGs, as well as six other bright galaxies around the core. As constraints we use the multiple images we found as listed in Merten et al. 2011 (MNRAS, 417, 333) where we published the first model of this cluster, with some revision as seen in the attached table/region file. We also acknowledge input from Johan Richard et al. for spectroscopic redshifts and their input for the revision of multiple images in the framework of the Frontier Field map making discussions. We note that the redshifts of 8 systems were left to be optimized by the model (namely those that had no spectroscopic redshifts to date): systems number 1,2,3,7,8,9,10,11 (see attached file). The best-fit model, resulting multiple image reproduction rms, is 2.06" (using the average source position), and the chi^2 is 84.78 using a positional error of sigma_pos=1.4". The number of constraints we use is n_c=44, the number of free parameters is n_p=15, so that the number of DOF=29. This results in a reduced chi^2 of chi^2/DOF=84.78/29=2.92.

PIEMD+eNFW: on top of the PIEMD galaxies, two eNFW halos are used whose centers we fix on the centers of the two BCGs. Five bright galaxies as are left to be freely weighted by the MCMC, and the same set of constraints as above is used. The best-fit model, resulting multiple image reproduction rms is 1.96" (using the average source position), and the chi^2 is 76.73 using a positional error of sigma_pos=1.4". The number of constraints we use is n_c=44, the number of free parameters is n_p=16, so that the number of DOF=28. This results in a reduced chi^2 of chi^2/DOF=76.73/28=2.74.

# The resulting best-fit parameter values and errors are given in the designated attached file.

# Multiple images and red-sequence selected galaxies, as mentioned, are attached as a ds9 region file or table.
